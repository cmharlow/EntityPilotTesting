...
def get_CUL_score(record_elems, resp_elems):
    #returns top result for all possible matching points.

    #if both are empty, ignore. covers lists + strings
    if record_elems is None or resp_elems is None:
        return None
    #if both are single strings, match + move on.
    elif isinstance(record_elems, str) and isinstance(resp_elems, str):
        score = str(fuzz.token_sort_ratio(record_elems, resp_elems))
        return score
    #only record elements is single string, resp is list
        ...
        for n in range(len(resp_elems)):
            ...
            scores.append(score)
        return max(scores)
    #only response elements is single string, record is list
